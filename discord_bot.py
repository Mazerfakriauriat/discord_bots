...
dsa = [804395649210646589]

def death():
	date = [str(random.randint(1,28)),str(random.randint(1,12)),str(random.randint(2021, 2117))]
	...
	"Ты заболеешь гриппом и из-за осложнений умрешь", "Ты подскользнешься и ударишься головой", "Ты умрешь во время операции", "Тебя изобьют в тюрьме", "Ты перевернешься на машине", "Ты умрешь от туберкулеза", "Ты будешь ремонтировать крышу и грохнешься"
...
class MyClient(discord.Client):
	...
	async def on_message(self, message):
		...
		if message.author.id == self.user.id:
			return
		elif message.author.id in dsa and message.content.startswith('!датасмерти'):
		...
		elif message.content.startswith('!датасмерти') and not message.author.id in dsa:
			dsa.append(message.author.id)
			reason = death()[1]
			date = death()[0]
			await message.channel.send("Мои пророки говорят, что ты умрешь {0}. {1}!".format(date, reason))
...
client = MyClient()
client.run('ODI0MjcwNjgwMzQxMjE3MzIx.YFs75A.gjpCDy2i7AZiAMGFiqNRWC87tSI')